...
export class WatchComponent implements AfterViewInit, OnDestroy {
    ...
    onVideoError(): void {
        this.error.set(
            'Video failed to load. Check that the file exists and is in a supported format (mp4, webm, mkv).',
        );
        this.clearProgressInterval();
    }
    ...
    private saveCurrentProgress(completed = false): void {
        ...
        if (pos === 0) return;
        this.watchService.saveProgress(ep.id, pos, completed || this.completedMarked).subscribe();
    }
